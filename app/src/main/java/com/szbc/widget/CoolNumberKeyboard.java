...
//			Toast.makeText(mContext, ""+Character.toString((char) primaryCode), Toast.LENGTH_SHORT).show();
			autoNextNumber();
		}
	};
	...
	private final View.OnClickListener mNumberSelectedHandler = new View.OnClickListener() {
		...
		public void onClick(View view) {
			...
			if (id == R.id.keyboard_number_0) {
				...
				if (mShowKeyboard != R.xml.keyboard_number_car) {
					mShowKeyboard = R.xml.keyboard_number_car;
...
//					Toast.makeText(mContext, ""+i.getText(), Toast.LENGTH_SHORT).show();
				}
			}
	...
	public CoolNumberKeyboard(Context context, boolean autoAlpha, boolean autoCommit, OnChoiceListener<String> mOnChoiceListener) {
		...
		mNumberKeyboard = new Keyboard(mContext, R.xml.keyboard_number_car);
...
//				Log.e("ZP","车牌输入法取消了");
		...
		mDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				// TODO 自动生成的方法存根
//				Log.e("ZP","车牌输入法dismiss了");
				//用EventBus通知HomeFragment车牌号对话框dismiss了
			}
		});
