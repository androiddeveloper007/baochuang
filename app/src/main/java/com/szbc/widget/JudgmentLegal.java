...
public class JudgmentLegal {
	 ...
	 * @return
	 */
	public static boolean isPwdChar(String post) {

		if(post.matches("^[a-zA-Z0-9]+[a-zA-Z0-9-]*[a-zA-Z0-9]+$") || post.matches("(\\S)+[@]{1}(\\S)+[.]{1}(\\w)+"))
		{
			return true;
		}else
		{
	...
	public static boolean isLength(String str, int startLength, int endLenght) {
		if (TextUtils.isEmpty(str)) {
			return false;
		}
	 ...
	 * @return
	 */
	public static boolean isPhone(String post) {
		if (TextUtils.isEmpty(post)) {
			return false;
		}
		return post
				.matches("^(0?1[3578]\\d{9})$|^((0(10|2[1-3]|[3-9]\\d{2}))?[1-9]\\d{6,7})$");
