...
public class SortAdapter extends BaseAdapter implements SectionIndexer {
	...
	public void update(List<CarBrand> list){
		this.list = list;
...
//				.setLoadingDrawableId(R.drawable.icon_stub)
				...
				.build();
		x.image().bind(viewHolder.iv, list.get(position).getBrandPictureUrl(), options);
		return convertView;
	}
	@Override
	...
	public int getPositionForSection(int sectionIndex) {
		for (int i = 0; i < getCount(); i++) {
			String sortStr = list.get(i).getInitialLetter();
	...
	public int getSectionForPosition(int position) {
		return list.get(position).getInitialLetter().charAt(0);
	}
